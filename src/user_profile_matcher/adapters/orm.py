...
class PlayerProfile(Base):
    ...
    modified = Column("modified", DateTime, default=datetime.now, onupdate=datetime.now)
    last_session = Column("last_session", DateTime, default=datetime.now, onupdate=datetime.now)
    ...
    gender = Column("gender", Enum("male", "female", "nonbinary", name="gender_enum", create_type=False))
    ...
    def get_dict(self):
        ...
        new_dict = {
            ...
            if not callable(value) and not attr.startswith("__") and not(attr == "_sa_instance_state")
        }
        return new_dict
